...
4. Try submitting a **Buy** transaction (type: Buy, fill in symbol, name, shares, and price)
5. The app crashes immediately
6. Even after fixing that crash, try searching or adding another transaction and watch the summary totals break
...
1. **Bug 1 (crash)**: A Buy transaction sets a field to `null` that a downstream function tries to call a method on. Look at how the form prepares data for different transaction types.
